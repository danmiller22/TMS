...
function useHotkey(k: string, cb: () => void) {
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === k.toLowerCase()) {
        e.preventDefault();
        cb();
      }
    };
...
function SearchModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const results = [
    ...trucks
      .filter(t => [t.id, t.vin ?? '', t.make ?? ''].some(x => x.toLowerCase().includes(q.toLowerCase())))
      .map(t => ({ type: 'Truck', id: t.id, to: '/trucks' })),
    ...trailers
      .filter(t => [t.id, t.owner ?? '', t.extCode ?? ''].some(x => x.toLowerCase().includes(q.toLowerCase())))
      .map(t => ({ type: 'Trailer', id: t.id, to: '/trailers' })),
    ...cases
      .filter(c => [c.id, c.title, c.assetId ?? ''].some(x => x.toLowerCase().includes(q.toLowerCase())))
      .map(c => ({ type: 'Case', id: c.id, to: '/cases' })),
  ].slice(0, 8);
      ...
      {open && (
        <motion.div className="fixed inset-0 z-50 flex items-center justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          <motion.div initial={{ y: 8, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 8, opacity: 0 }} transition={{ type: 'spring', stiffness: 220, damping: 22 }} className="relative z-10 w-full max-w-xl rounded-2xl border border-border p-4 glass">
            ...
            <div className="mt-3">
              {results.length === 0 ? (
                <div className="text-sm opacity-60 px-1 py-2">No results</div>
              ) : (
                results.map((r, i) => (
